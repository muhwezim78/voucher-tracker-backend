...
class MikroTikManager:
    ...
    def create_voucher(
        ...
        try:
            ...
            logger.info(
                f"Voucher {code} created with profile {profile_name} and uptime {uptime_limit}"
            )
            return True
        except Exception as e:
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if system_info and identity_info:
                model = "Unknown"
                if routerboard_info and len(routerboard_info) > 0:
                    model = routerboard_info[0].get("model", "Unknown")
                    if model == "Unknown":
                        model = system_info[0].get("board-name", "Unknown")

                architecture = system_info[0].get("architecture-name", "")
                ...
                return {
                    ...
                    "serial_number": (
                        routerboard_info[0].get("serial-number", "")
                        if routerboard_info
                        else ""
                    ),
                    "firmware": system_info[0].get("factory-software", ""),
                    "platform": system_info[0].get("platform", ""),
                }
            return {}
        except Exception as e:
            ...
            try:
                ...
                if system_info:
                    return {
                        ...
                        "model": system_info[0].get("board-name", "Unknown"),
                        "architecture": system_info[0].get("architecture-name", ""),
                        "error": "Partial data - some endpoints unavailable",
                    }
